...
const addReview = async (req, res) => {
  try {
    ...
    if (user.isadmin) {
      return res
        .status(404)
        .json({ message: "Los administadores no pueden comentar reviews" });
    } else {
      ...
      if (user.products[0] === undefined) {
        ...
        for (let productUser of user.products) {
          if (
            productUser.reviews.userId === parseInt(idUser) &&
            productUser.reviews.productId === parseInt(idProduct)
          ) {
            return res
              .status(409)
              .json({ message: "Ya realizo una review en este producto" });
          }
        }
...
const updateReview = async (req, res) => {
  try {
    ...
    if (review) {
      ...
      return res
        .status(404)
        .json([{ message: "La review no corresponde con el producto" }]);
    } else {
...
const deleteReview = async (req, res) => {
  try {
    ...
    if (review) {
      ...
      return res
        .status(404)
        .json({ message: "La review no coincide con el producto" });
    } else {
...
const getReview = async (req, res) => {
  try {
    ...
    if (reviews[0] !== undefined) {
      ...
      return res
        .status(404)
        .json({ message: "No hay reviews en este producto" });
    }
  } catch (error) {
    console.log(error);
    res.status(500).json([{ Error: "something was wrong" }]);
